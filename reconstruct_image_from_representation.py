...
def neural_style_transfer(config):
    ...
    out_dir_name = f"combined_{os.path.splitext(config['content_img_name'])[0]}_{os.path.splitext(config['style_img_name'])[0]}"
    ...
    print(f'Using {config["model"]} for optimization.')

    content_img_features = neural_net(content_img)
    style_img_features = neural_net(style_img)

    target_content_representation = content_img_features[content_feature_maps_index_name[0]].squeeze(axis=0)
    target_style_representation = [
        utils.gram_matrix(x) for cnt, x in enumerate(style_img_features) if cnt in style_feature_maps_indices_names[0]
    ]
    target_representations = [target_content_representation, target_style_representation]

    num_of_iterations = {
        "lbfgs": 50,  # Reduced from 200
        "adam": 100   # Reduced from 500
    }
    ...
    if config['optimizer'] == 'adam':
        ...
        for cnt in range(num_of_iterations[config['optimizer']]):
            ...
            if cnt % config['print_freq'] == 0 or cnt == num_of_iterations[config['optimizer']] - 1:
                with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--content_img_name", type=str, default='figures.jpg')
    ...
    parser.add_argument("--tv_weight", type=float, default=1e-1)
    ...
    parser.add_argument("--print_freq", type=int, default=10)  # New argument
